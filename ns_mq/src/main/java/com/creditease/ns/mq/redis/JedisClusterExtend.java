...
public class JedisClusterExtend implements JedisCommands, BasicCommands, Closeable {
    ...
    public JedisClusterExtend(Set<HostAndPort> nodes, int timeout) {
        this(nodes, timeout, 5);
    }

    public JedisClusterExtend(Set<HostAndPort> nodes) {
        this(nodes, 2000);
    }

    public JedisClusterExtend(Set<HostAndPort> nodes, int timeout, int maxRedirections) {
        this(nodes, timeout, maxRedirections, new GenericObjectPoolConfig());
    }

    public JedisClusterExtend(Set<HostAndPort> nodes, GenericObjectPoolConfig poolConfig) {
        this(nodes, 2000, 5, poolConfig);
    }

    public JedisClusterExtend(Set<HostAndPort> nodes, int timeout, GenericObjectPoolConfig poolConfig) {
        this(nodes, timeout, 5, poolConfig);
    }

    public JedisClusterExtend(Set<HostAndPort> jedisClusterNode, int timeout, int maxRedirections, GenericObjectPoolConfig poolConfig) {
        this(jedisClusterNode, timeout, timeout, maxRedirections, poolConfig);
    }

    public JedisClusterExtend(Set<HostAndPort> jedisClusterNode, int connectionTimeout, int soTimeout, int maxRedirections, GenericObjectPoolConfig poolConfig) {
    ...
    public List<byte[]> brpop(final int timeout, final byte[] key) {
        return (List)(new JedisClusterCommand<List<byte[]>>(this.connectionHandler, this.maxRedirections) {
            public List<byte[]> execute(Jedis connection) {
                return connection.brpop(timeout, new byte[][]{key});
            }
    ...
    public byte[] rpop(final byte[] key) {
        return (byte[])(new JedisClusterCommand<byte[]>(this.connectionHandler, this.maxRedirections) {
    ...
    public Object eval(final byte[] script, byte[] key, final int keyCount, final byte[]... params) {
        return (new JedisClusterCommand<Object>(this.connectionHandler, this.maxRedirections) {
            public Object execute(Jedis connection) {
                return connection.eval(script, Protocol.toByteArray(keyCount), params);
            }
        }).run(SafeEncoder.encode(key));
    }
